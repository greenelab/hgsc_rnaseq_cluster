...
model <- coxph(Surv(months, vital) ~ (factor(Dataset)+ factor(age) + strata(factor(FewerStage))), AnalSet)
...
coxSum <- function (coxData, name) {
  ...
  CoxPHtmploc = file.path(fig_dir, "AllData_COXPHoutput_nodeb.csv")
...
model3 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + factor(age) + strata(factor(FewerStage))), SurvAACESB)
...
coxSum <- function (coxData, name) {
  ...
  CoxPHtmploc = file.path(fig_dir, "AACESBCOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
model4 <- coxph(Surv(months, vital) ~ (factor(age) + strata(factor(FewerStage))), SurvAACESB)
...
model5 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + factor(age) + strata(factor(FewerStage))), SurvAACESW)
...
coxSum <- function (coxData, name) {
  ...
  # Combine together and write to file
  CoxPHtmp <- cbind(conf.int, hazard, Pvalues)
  CoxPHtmploc = file.path(fig_dir, "AACESWCOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
#Likelihood ratio test of null hypothesis that there 
#is no association between K4 Cluster and Survival, accounting for age
model6 <- coxph(Surv(months, vital) ~ (factor(age) + strata(factor(FewerStage))), SurvAACESW)
...
model7 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + factor(age) + strata(factor(FewerStage))), TCGAset)
...
coxSum <- function (coxData, name) {
  ...
  # Combine together and write to file
  CoxPHtmp <- cbind(conf.int, hazard, Pvalues)
  CoxPHtmploc = file.path(fig_dir, "TCGACOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
#Likelihood ratio test of null hypothesis that there 
#is no association between K4 Cluster and Survival, accounting for age
model8 <- coxph(Surv(months, vital) ~ (factor(age) + strata(factor(FewerStage))), TCGAset)
...
model9 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + factor(age) + strata(factor(FewerStage))), MAYOset)
...
coxSum <- function (coxData, name) {
  ...
  # Combine together and write to file
  CoxPHtmp <- cbind(conf.int, hazard, Pvalues)
  CoxPHtmploc = file.path(fig_dir, "MAYOCOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
#Likelihood ratio test of null hypothesis that there 
#is no association between K4 Cluster and Survival, accounting for age
model10 <- coxph(Surv(months, vital) ~ (factor(age) + strata(factor(FewerStage))), MAYOset)
...
model11 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + factor(age) + strata(factor(FewerStage))), TOTHset)
...
coxSum <- function (coxData, name) {
  ...
  # Combine together and write to file
  CoxPHtmp <- cbind(conf.int, hazard, Pvalues)
  CoxPHtmploc = file.path(fig_dir, "TOTHCOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
#Likelihood ratio test of null hypothesis that there 
#is no association between K4 Cluster and Survival, accounting for age
model12 <- coxph(Surv(months, vital) ~ (factor(age) + strata(factor(FewerStage))), TOTHset)
...
model13 <- coxph(Surv(months, vital) ~ (factor(ClusterK4_kmeans) + strata(factor(FewerStage))), YOSHset)
...
coxSum <- function (coxData, name) {
  ...
  # Combine together and write to file
  CoxPHtmp <- cbind(conf.int, hazard, Pvalues, WaldsP)
  CoxPHtmploc = file.path(fig_dir, "YOSHCOXPHoutput_nodeb.csv")
  write.csv(CoxPHtmp, file=CoxPHtmploc)
}
...
#Likelihood ratio test of null hypothesis that there 
#is no association between K4 Cluster and Survival, accounting for age
model14 <- coxph(Surv(months, vital) ~  (strata(factor(FewerStage))), YOSHset)
