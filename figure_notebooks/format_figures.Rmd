...
Using the PDFs generated in the other notebooks, we will put them together into multi-panel figures.


### format Figure 2 -- PCA of centroids
...
centroids_k3_gg = image_read_pdf(file.path(proj_dir,
                                  "/figure_notebooks/manuscript_figs/centroids_k3.pdf"))
centroids_k3_gg = image_border(centroids_k3_gg, "white", "130x10")
centroids_k3_gg = image_annotate(centroids_k3_gg, "a", size = annot_size, location = "+0-50")


centroids_k4_gg = image_read_pdf(file.path(proj_dir,
                                  "/figure_notebooks/manuscript_figs/centroids_k4.pdf"))
centroids_k4_gg = image_border(centroids_k4_gg, "white", "130x10")
centroids_k4_gg = image_annotate(centroids_k4_gg, "b", size = annot_size, location = "+0-50")



full_panel <- image_append(c(centroids_k3_gg,
                             centroids_k4_gg), stack = TRUE)
full_panel


outfile = paste0(proj_dir, "/figure_notebooks/manuscript_figs/final_figs/main_centroid_fig.pdf")
image_write(full_panel, format = "pdf", outfile)

```

### format Figure 1 -- all sankeys (old)
...
aaces_sankey_ggAA = image_read(file.path(proj_dir,
                                  "/figure_notebooks/manuscript_figs/sankey_aaces_B.png"))
...
yoshihara_sankey_gg = image_annotate(yoshihara_sankey_gg, "e", size = annot_size)
...
tothill_sankey_gg = image_annotate(tothill_sankey_gg, "f", size = annot_size)
...
bot_panel = image_append(c(yoshihara_sankey_gg, tothill_sankey_gg), stack = FALSE)
...
```{r format_fig1, echo=TRUE, warning=FALSE, message=F}
...
```{r make_supp_kmeans_nmf, echo=TRUE, warning=FALSE, message=F}
...
aaces_kmeans_NMF_ggW = image_annotate(aaces_kmeans_NMF_ggW, "b", size = annot_size)
...
bot_panel = image_append(c(yoshihara_kmeans_NMF_gg, tothill_kmeans_NMF_gg), stack = FALSE)
...
```{r make_supp_dataset_comparison, echo=TRUE, warning=FALSE, message=F}
...
```{r qc_supp, echo=TRUE, warning=FALSE, message=F}
...
top_panel = image_annotate(top_panel, "a", size = annot_size)
top_panel = image_annotate(top_panel, "b", size = annot_size, location = "+1200+0")
top_panel
...
```{r cluster_qc_supp, echo=TRUE, warning=FALSE, message=F}
