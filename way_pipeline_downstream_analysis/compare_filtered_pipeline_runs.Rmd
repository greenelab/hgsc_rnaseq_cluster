...
In this notebook I compare the clusters from the two runs of the pipeline.
The runs differ in the fact that they either have or don't have samples with neoadjuvant therapy.
Since the clusters are mapped to TCGA clusters, we can use the cluster IDs to compare them.
To do this, I treated the cluster IDs as expected (with neoadj) and predicted (withOUT neoadj) labels.
...
```{r plot_aaces.eset, fig.align='center', fig.width=15, fig.height=15, echo=TRUE, warning=FALSE, message=F}
...
```{r plot_aaces_nmf, fig.align='center', fig.width=15, fig.height=15, echo=TRUE, warning=FALSE, message=F}
